...
const createStatus = async (data: {
    ...
    try {
        ...
        const followerIds = follows.map((f) => (f.user as any).toString());

        if (followerIds.length > 0) {
            const stationName = station.name || "the station";
            const title = `New status from ${stationName}`;
            const body = data.content.length > 100
            ...
            await NotificationService.sendBulkNotifications(
                ...
                body,
                "announcement",
                { stationId: data.stationId, statusId: (status as any)._id.toString(), type: "new_status" },
            );
        }
    } catch (error) {
        logger.error("[StatusService] Failed to notify followers about new status", { error });
    }
        ...
        if (!stationMap.has(stationId)) {
            stationMap.set(stationId, {
                station: {
                    ...
                    isActive: status.stationIsActive ?? false,
                    isVerified: status.stationIsActive ?? false,
                },
                statuses: [],
            });
        }
